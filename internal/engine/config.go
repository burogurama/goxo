// Config assembly: the engine builds the bus and runner configs it hands to its
...
type Params struct {
	...
	Timeout   time.Duration // per-message handler timeout
}
...
// prefer the settings selectors over the definition's.
func busConfig(m *manifest.Manifest, s *settings.Settings) bus.Config {
	return bus.Config{
		URL:      s.BusURL,
...
// runnerConfig briefs one handler run: the command to spawn, the agent identity,
// the merged args the handler receives as config, and the declared I/O. The name
// and outputs come from the definition; the inputs prefer the settings selectors
// over the definition's.
func runnerConfig(m *manifest.Manifest, s *settings.Settings, p Params) (runner.Config, error) {
	var (
		cfg map[string]any
		err error
	)
